        ...
        public void OnGet(long id)
        {
            ...
            foreach (var exposer in _exposers)
            {
                ...
                foreach (var (key, value) in exposedPermissions)
                {
                    ...
                    foreach (var item in from permission in value
                                         let item = new SelectListItem(permission.Name , permission.Code.ToString())
                                         {
                                             Group = @group
                                         }
                                         where Command.MappedPermissions.Any(x => x.Code == permission.Code)
                                         select item)
                    {
                        item.Selected = true;
